<script>
	...
	const calculateScore = () => {
		return selectedQuestions.filter(({ answer, selectedAnswer }) => answer === selectedAnswer)
			.length;
	};
	...
	const selectOption = (selectedAnswer, question) => {
		...
		if (index !== -1) {
			...
			// Add a new question to the selectedQuestions array
			selectedQuestions = [
				...selectedQuestions,
				{
					...question,
...
<div class="min-h-screen flex items-center justify-center">
	<div
		class="border border-gray-600 w-full max-w-[500px] min-h-40 bg-black rounded-3xl p-6 shadow-xl"
	>
		...
		{#if isDone}
			...
			<div class="flex justify-end mt-8">
				<button
					...
					class="py-3 px-10 rounded-xl text-white bg-purple-700 font-medium"
				>
					Reset
				</button>
			</div>
		{:else}
			<div>
					...
					{#if activeQuestion === index}
						...
						<div class="mt-5 space-y-5">
							{#each question.options as option, idx (idx)}
								...
								<button
									class="w-full h-14 border border-gray-600 text-gray-200 flex items-center rounded-xl px-6"
									class:selected-option={isSelected}
									on:click={() => selectOption(option, question)}
								>
									{option}
								</button>
							{/each}
				...
				<div class="flex justify-end mt-8">
					<button
						class="font-medium text-white px-6"
						on:click={handleBack}
